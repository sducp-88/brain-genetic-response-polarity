#!/usr/bin/env python3
"""QC donor-by-class RADC pseudobulk counts before disease modeling."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/phase1/radc_pseudobulk_qc"),
    )
...
def main() -> None:
    ...
    pca_figure, pca_axes = plt.subplots(2, 4, figsize=(18, 9))
    library_figure, library_axes = plt.subplots(2, 4, figsize=(18, 9))
    color_map = {
        "Yes": "#D55E00",
        "No": "#0072B2",
    ...
    for class_index, class_name in enumerate(class_order):
        h5ad_path = pseudobulk_dir / f"RADC_{class_name}_pseudobulk_counts.h5ad"
        ...
        pca_frame["AD_status"] = eligible_obs["AD_status"].astype(str).to_numpy()
        ...
        for status in ["No", "Yes", "Not Evaluated"]:
            selected = pca_frame["AD_status"].eq(status)
            pca_axis.scatter(
        ...
        statuses = data.obs["AD_status"].astype(str)
        for status in ["No", "Yes", "Not Evaluated"]:
    ...
    pca_figure.legend(handles, labels, loc="upper center", ncol=3)
    pca_figure.suptitle(
        "RADC donor-level pseudobulk PCA by cell class", y=0.99
    )
    ...
    library_figure.legend(handles, labels, loc="upper center", ncol=3)
